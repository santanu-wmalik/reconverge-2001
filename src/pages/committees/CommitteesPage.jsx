            ...
            <GlassCard className="h-full border-gold-500/10 hover:border-gold-500/30 transition-colors duration-300">
              <div className="flex items-start gap-3 mb-3">
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="text-base font-heading font-bold text-ink dark:text-white leading-tight">
                    {committee.name}
                  </h3>
              ...
              <p className="text-ink-muted dark:text-slate-400 text-sm leading-relaxed mb-4">
                {committee.description}
              </p>

              {committee.lead !== null && (
                <>
                  <div className="h-px bg-forest-500/15 dark:bg-white/5 mb-3" />
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-semibold uppercase tracking-wider text-gold-700 dark:text-gold-400">Lead</span>
                    <span className={`text-xs ${String(committee.lead).toLowerCase().includes('tbd') ? 'text-amber-700 dark:text-amber-400 italic' : 'text-ink-soft dark:text-slate-300 font-medium'}`}>
                      {committee.lead}
                    </span>
                  </div>
                  {committee.coLead && (
                    <div className="flex items-center gap-2 mt-1">
                      <span className="text-xs font-semibold uppercase tracking-wider text-gold-700 dark:text-gold-400/70">Co-lead</span>
                      <span className="text-ink-muted dark:text-slate-400 text-xs">{committee.coLead}</span>
                    </div>
                  )}
                  {committee.members && committee.members.length > 0 && (
                    <div className="flex items-start gap-2 mt-1">
                      <span className="text-xs font-semibold uppercase tracking-wider text-gold-700 dark:text-gold-400/70 flex-shrink-0">Members</span>
                      <span className="text-ink-muted dark:text-slate-400 text-xs">
              ...
              <p className="text-xs text-ink-muted dark:text-slate-500 mt-3">
                {(() => {
                  const filled =
          ...
          <motion.div key={br.branch} variants={staggerItem}>
            <GlassCard className={`border-l-4 ${br.status === 'open' ? 'border-l-amber-500/60' : 'border-l-emerald-500/60'}`}>
              <div className="flex items-center justify-between mb-2">
                <h4 className="text-ink dark:text-white font-semibold text-sm">{br.branch}</h4>
                {br.status === 'open' ? (
              ...
              {br.reps.length > 0 ? (
                <div className="flex flex-wrap gap-2 mb-2">
                  {br.reps.map((rep) => (
                    <span
                      key={rep}
                      className="inline-flex items-center gap-1.5 bg-white dark:bg-white/5 rounded-lg px-2.5 py-1 border border-forest-500/15 dark:border-white/5"
                    >
                      <svg className="w-3.5 h-3.5 text-gold-700 dark:text-gold-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                      </svg>
                      <span className="text-ink-soft dark:text-slate-300 text-xs font-medium">{rep}</span>
                    </span>
                  ))}
                </div>
              ) : (
                <p className="text-amber-700/90 dark:text-amber-400/80 text-xs italic mb-2">
        ...
        {stateChampions.map((sc) => (
          <motion.div key={sc.state} variants={staggerItem}>
            <GlassCard className="border-l-4 border-l-amber-500/60">
              <div className="flex items-center justify-between mb-2">
                <h4 className="text-ink dark:text-white font-semibold text-sm">{sc.state}</h4>
                <Badge variant="warning" size="sm">Volunteer Needed</Badge>
              </div>
              <p className="text-xs text-ink-muted dark:text-slate-500">{sc.note}</p>
            </GlassCard>
          </motion.div>
        ))}
      </motion.div>
      ...
      >
        <GlassCard className="border-amber-500/20 bg-gradient-to-br from-amber-500/5 to-primary-900/20">
          <div className="text-center max-w-xl mx-auto">
            ...
            <h3 className="text-xl font-heading font-bold text-ink dark:text-white mb-2">Help Us Find Missing Batchmates</h3>
            <p className="text-ink-muted dark:text-slate-400 text-sm leading-relaxed mb-4">
        ...
        <GlassCard className="text-center border-gold-500/20 bg-gradient-to-br from-gold-500/5 to-primary-900/20">
          <div className="max-w-xl mx-auto">
            <div className="w-14 h-14 rounded-full bg-gold-500/10 border border-gold-500/20 flex items-center justify-center mx-auto mb-5">
              <svg className="w-7 h-7 text-gold-700 dark:text-gold-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
              </svg>
            </div>
            <h3 className="text-2xl font-heading font-bold text-ink dark:text-white mb-3">
              Want to lend a hand?
            </h3>
            <p className="text-ink-muted dark:text-slate-400 leading-relaxed mb-6">
