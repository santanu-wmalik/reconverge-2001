...
export default function NewsPage() {
  return (
    <motion.div {...pageTransition}>
          ...
          <motion.div key={article.id} variants={staggerItem}>
            <GlassCard>
              ...
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-xs text-ink-muted">{formatDate(article.date)}</span>
              </div>
              <h3 className="text-ink font-semibold">{article.title}</h3>
              <p className="text-ink-soft text-sm mt-1">{article.excerpt}</p>
      ...
      {/* Sponsors */}
      <h3 className="text-xl font-heading font-bold text-ink mb-6">Our Sponsors</h3>
