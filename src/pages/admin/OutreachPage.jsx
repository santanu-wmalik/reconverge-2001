...
export default function OutreachPage() {
  ...
  return (
    <motion.div {...pageTransition}>
      <div className="mb-8">
        <h1 className="text-3xl font-heading font-bold text-ink">Outreach & Alumni Tracking</h1>
        <p className="text-ink-soft mt-1">Per-branch WhatsApp group sizes (as reported by volunteers), named branch reps, and flagged missing alumni.</p>
      </div>
      ...
      {/* Per-Branch Detail */}
      <h2 className="text-lg font-heading font-bold text-ink mb-4">Branch-wise Outreach Details</h2>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <h3 className="text-ink font-semibold">{branch.branch}</h3>
                    <Badge
                      variant={
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-3">
                    <div>
                      <p className="text-xs text-ink-muted">WhatsApp group size</p>
                      <p className="text-sm text-ink font-medium">{branch.inGroup || <span className="text-ink-muted">Not reported</span>}</p>
                    </div>
                    <div>
                      <p className="text-xs text-ink-muted">Branch representatives</p>
                  ...
                  {branch.notes && (
                    <p className="text-xs text-ink-muted mt-3 italic">{branch.notes}</p>
                  )}

                  {/* Missing Alumni */}
                  {branch.missingKnown && branch.missingKnown.length > 0 && (
                    <div className="mt-3 pt-3 border-t border-forest-500/15">
      ...
      >
        <GlassCard className="border-gold-500/20 text-center">
          <p className="text-ink-soft text-sm">
            ...
            or email <a href="mailto:crec2001reunion@gmail.com" className="text-gold-700 hover:text-gold-300">crec2001reunion@gmail.com</a>.
          </p>
        </GlassCard>
      </motion.div>
    </motion.div>
